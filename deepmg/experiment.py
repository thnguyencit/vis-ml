...
def validation_para(options):
    ...
    if not (options.type_emb in ['raw','bin','fill','fills','zfill','zfills','tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom','none']):
        print  ('you entered --type_emb=' + str(options.type_emb))
        print  (utils.textcolor_display('--type_emb') + " only supports 'raw','bin','fill','fills','zfill','zfills','tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom")
        valid = 0
    ...
    if not (options.type_bin in ['spb','pr','eqw','eqf','kmeans','kmedoids','']):
...
def read_data (options, suffix = '', included_labels = 'y' , get_names_features_samples = 'n', mode='internal') :
    '''
    Read data
    ...
    ** orderf_fill used if and only if type_emb == 'fill' or a type of fill
    ...
    if get_names_features_samples in ['y','yes']:
        name_features =  list(v_data_ori.index)
        name_samples =  list(v_data_ori.columns.values)
    ...
    if get_names_features_samples in ['y','yes']:
        return v_data_ori, v_labels, name_features, name_samples
    else:
        return v_data_ori, v_labels
    
def create_fillup_folder (options,path_write_parentfolder_img,
        cordi_x,cordi_y,data,labels, prefix_img = 'fill', only_visual = 'n', suf_after_i=''):
    '''
    ...
    if only_visual in ['n']:
        data = utils.load_img_util(num_sample = len(data), pattern_img= prefix_img,
                ...
                channel = options.channel,mode_pre_img = options.mode_pre_img,type_emb=options.type_emb,suf_after_i=suf_after_i)       
...
def reduce_dim (options,prefix_file_log, seed_value, index, train_x, val_x, v_data, train_y=None, val_y=None):
    ...
    if options.algo_redu ==  "rd_pro": #if random projection
        ...
        if options.log_file == '' or options.log_file == None:
            options.log_file = os.path.join(pathlib.Path.cwd(),  'log.txt')
...
def visual_section (options,  train_x,val_x, train_y, val_y, v_data, v_labels,train_x_original,
          prefix_file_log='', path_write_parentfolder_img='', cordi_x=[],cordi_y=[],train_test_whole = 'n',seed_value=1, index=1,suf_after_i=''):
    '''
    ...
    arg:
        ...
        suf_after_i: provide sample names to suffix for pictures names

    return    train_x,val_x,v_data (numerical data)
    ...
    if options.type_emb != 'none': # added by tbtoan   
            ...
            if (not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png")) or options.recreate_img > 0 :
                ...
                if options.orderf_fill == 'high':  
                    ...
                    for i in range(0, len(train_y)):
                        ...
                        #print (train_high)  
                        # 
                        if options.type_run not in ['predict']:
                    ...
                    for i in range(0, len(val_y)):
                        ...
                        if suf_after_i != '':
                            vis_data.generate_image (data = [x for _,x in sorted(zip(mean_spe,val_x[i]))], bins_breaks = bins_breaks, colormap = options.colormap, 
                                file_name_emb = path_write + "/val_" + str(i) + str(suf_after_i[i]),size_p = options.point_size, fig_size = fig_v_size,
                                marker = options.shape_drawn, alpha_v = options.alpha_v,
                                cmap_vmin = options.cmap_vmin, cmap_vmax = options.cmap_vmax, margin= options.margin, 
                                min_v = min_v_temp, num_bin = options.num_bin,
                                cor_x=cordi_x,cor_y=cordi_y)  
                    ...
                    for i in range(0, len(val_y)):
                        ...
                        if suf_after_i != '':
                            ...
                            vis_data.generate_image (data = val_x[i], bins_breaks = bins_breaks, colormap = options.colormap, 
                                ...
                                min_v = min_v_temp, num_bin = options.num_bin,
                                cor_x=cordi_x,cor_y=cordi_y) 
                if (not (os.path.exists(path_write+'/train_y.csv'))):
                    print("hhhhhhhhhhhhhhhh")
                    print(path_write)
                    print(os.path.exists(path_write+'/train_y.csv'))
        ...
        elif options.type_emb in  ['tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom'] : #if use other embeddings (not fill), then now embedding!
    ...
    val_x_img = utils.load_img_util(num_sample = len(val_y), pattern_img='val',
        ...
        channel = options.channel,mode_pre_img = options.mode_pre_img, type_emb=options.type_emb,data=val_x,suf_after_i=suf_after_i)
    
    # if predict, so we do not need to generate images for training
    if options.type_run not in ['predict']:
...
def run_kfold_deepmg(options,args):
    ...
    if options.type_run not in ['vis','visual']:  
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location("models_def", "models_def.py")
            models_def = importlib.util.module_from_spec(spec)
            from models_def import EarlyStopping_consecutively
        import keras as kr
       
        try:
            from keras.applications.resnet50 import ResNet50, preprocess_input
        except:
            print('resnet50 has errors')
    ...
    # added by tbtoan END ### --------### ---------
        ...
        data, labels, name_features, name_samples = read_data(options,get_names_features_samples = 'y')  
        print('name_samples========')  
        print(name_samples)
    ...
    if options.test_ext == 'y': #if use external validation set
        v_data_ori, v_labels = read_data(options,suffix='z',get_names_features_samples = 'n', mode = 'external')
    ...
    for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.run_time) :
        ...
        for index, (train_indices,val_indices) in enumerate(skf.split(data, labels)):
            ...
            if not (options.algo_redu in  ['','none']): #if use reducing dimension
                train_x, val_x, v_data = reduce_dim (options,prefix_file_log, seed_value, index, train_x, val_x, v_data,train_y, val_y)
            ...
            if options.type_run in ['vis','visual']: #only visual, not learning
                ...
                if options.save_folds == 'y':
                    ...
                    #title_cols = np.array([["train_acc","val_acc","train_auc","val_auc","val_mcc","train_loss","val_loss","val_mcc","tn", "fp", "fn", "tp",
                    #    "time","ep_stopped", "index_fold","seed"]])
                    f=open(prefix_file_log+"_eachfold.txt",'a')
    ...
    if options.type_run in ['vis','visual']: #only visual, not learning
        ...
        f=open(prefix_file_log+"_sum.txt",'a')
        title_cols = np.array([['time','tr_ac',"va_ac","sd","va_au","sd","tn","fn","fp","tp","preci","sd","recall","sd","f1","sd","mcc","sd","epst"]])
        np.savetxt(f,title_cols, fmt="%s",delimiter="\t")   
        TN=auc_mean[6]
        FN=auc_mean[7]
        FP=auc_mean[8]
...
def run_holdout_deepmg(options,args, special_usecase = '', txt_time_pre =''):
    ...
    if options.type_run not in ['vis','visual']:  
        ...
        try:
            ...
            print('it has still issues with resnet50')
        from keras.applications.vgg16 import VGG16
        from keras.models import Model
    ...
    if special_usecase in ['predict'] and not os.path.isfile(os.path.join(options.original_data_folder , options.data_name + '_y.csv')):
        data, labels, name_features, name_samples = read_data(options,included_labels='n',get_names_features_samples = 'y')    
        print('name_samples========predict without labels')  
        print(name_samples)  
    else:           
        data, labels, name_features, name_samples = read_data(options,get_names_features_samples = 'y')   
        print('name_samples========predict with labels')  
        print(name_samples)   
    ...
    for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.whole_run_time) :
        ...
        for index in range(0,1):
            ...
            if options.type_run  in ['vis','visual']:  
                ...
                if special_usecase in ['predict']:   
                    ...
                    if len( labels) > 0 :#if existing the label
                        ...
                        # nthai: add samples name , 09/05/2021
                        #title_cols = np.array([["y_predicted",'rounded','y_ground_truth']])
                        title_cols = np.array([['sample_name',"y_predicted",'rounded','y_ground_truth']])
                        ...
                        np.savetxt(f,np.c_[name_samples, Y_pred_v_proba,Y_pred_v_proba.round(),val_y], fmt="%s",delimiter="\t")
                        ...
                        title_cols = np.array([['sample_name',"y_predicted",'rounded']])
                        np.savetxt(f,title_cols, fmt="%s",delimiter="\t") 
                        
                        # nthai: add samples name , 09/05/2021
                        #np.savetxt(f,np.c_[Y_pred_v_proba[:,1],Y_pred_v_proba[:,1].round()], fmt="%s",delimiter="\t")
                        np.savetxt(f,np.c_[name_samples,Y_pred_v_proba[:,1],Y_pred_v_proba[:,1].round()], fmt="%s",delimiter="\t")
                        f.close() 
            ...
            if options.type_run in ['vis','visual']: #only visual, not learning
                ...
                title_cols = np.array([["after training"]])           
                np.savetxt(f,title_cols, fmt="%s",delimiter="\t")
                title_cols = np.array([["t_acc","v_acc","t_auc","v_auc","t_los","v_los",'val_mcc',"tn", "fp", "fn", "tp","time","ep_stopped"]])           
                np.savetxt(f,title_cols, fmt="%s",delimiter="--")
                np.savetxt(f,np.around(np.c_[( 
                    train_acc[ep_stopped-1],val_acc[ep_stopped-1] , 
                    train_auc_score,val_auc_score,                    
